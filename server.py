...
def load_images():
    parents_json = requests.get(f"http://{config.get("WEB")}/parents/json")
    parents = parents_json.json()
    for parent in parents:
        photo_request = requests.get(f"http://{config.get("WEB")}/parent/photo/{parent['id']}")
        if not os.path.exists("imgs"):
            os.makedirs("imgs")
        path = os.path.abspath(os.path.join("imgs", f"{parent['id']}.jpg"))
        with open(path, 'wb') as f:
            f.write(photo_request.content)
        image = face_recognition.load_image_file(path)
        face_encoding = face_recognition.face_encodings(image)[0]
        known_face_encodings.append(face_encoding)
        known_face_names.append(f"{parent['id']}")
